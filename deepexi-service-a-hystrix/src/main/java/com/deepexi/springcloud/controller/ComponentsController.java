...
public class ComponentsController {
    ...
    public JSONObject getValue(@RequestBody Components components) {
        ...
        if (list.size() < 0) {
            throw new RuntimeException("该ID：" + components.getCOM_ID() + "没有对应的信息");
        }
        json.put("list", list);
        return json;
    }

    public JSONObject hystrixMethod(@RequestBody Components components) {
        JSONObject json = new JSONObject();
        json.put("list", "该ID：" + components.getCOM_ID() + "没有对应的信息" + "null---@HystrixCommand");
        return json;
    }
